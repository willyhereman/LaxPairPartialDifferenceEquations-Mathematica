...
laxPairMatrixL = {{(k-p)*(k*p-x[1,0,0])+k*x[0,0,0], -p*(k*(k-p)*(k^2 - k*p + p^2 - x[0,0,0] - x[1,0,0]) + x[0,0,0]*x[1,0,0])},
                  {p, -((k-p)*(k*p-x[0,0,0])+k*x[1,0,0])}};
laxPairMatrixM = {{(k-q)*(k*q-x[0,1,0])+k*x[0,0,0], -q*(k*(k-q)*(k^2 - k*q + q^2 - x[0,0,0] - x[0,1,0]) + x[0,0,0]*x[0,1,0])},
